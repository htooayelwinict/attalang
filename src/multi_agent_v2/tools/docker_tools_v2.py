...
WORKSPACE_ENV_VAR = "MULTI_AGENT_DOCKER_V2_WORKSPACE"
WORKSPACE_DEFAULT = "/tmp/multi-agent-docker-v2-workspace"
...
def _parse_ports(value: str | None) -> dict[str, int] | None:
    """Parse port mappings from various formats.

    Accepts:
    - JSON object: {"80/tcp": 9000} or {"80": 9000}
    - String format: "9000:80" or "9000:80/tcp"

    Returns normalized format: {"80/tcp": 9000}
    """
    if value is None:
        return None
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            for container_port, host_port in parsed.items():
                # Normalize container port to include /tcp if missing
                port_key = container_port
                if "/" not in port_key:
                    port_key = f"{port_key}/tcp"

                # Handle string or int host port
                if isinstance(host_port, str):
                    host_port = int(host_port)
                normalized[port_key] = int(host_port)
            return normalized
    except (json.JSONDecodeError, ValueError):
        pass

    # Try string format "HOST:CONTAINER" or "HOST:CONTAINER/protocol"
    if ":" in value:
        ...
        if len(parts) == 2:
            try:
                ...
                if "/" in container_part:
                    return {container_part: host_port}
                else:
                    return {f"{container_part}/tcp": host_port}
            except ValueError:
                pass

    raise ValueError(
        f"Invalid port format: {value}. "
        "Use JSON {{'80/tcp': 9000}} or string '9000:80'"
    )
...
def register_tools_on_agent(agent: Any, tools: list[Any] | None = None) -> list[str]:
    selected = list(tools) if tools is not None else list(ALL_DOCKER_TOOLS)
    registered: list[str] = []
...
def _wrap_tool_for_context(fn: Any) -> Any:
    """Wrap a function to accept RunContext as first parameter."""
    import functools
    import inspect
    ...
    # Create new signature with ctx as first parameter
    ...
    new_params = [ctx_param] + list(orig_sig.parameters.values())
    new_sig = orig_sig.replace(parameters=new_params)

    @functools.wraps(fn)
    def wrapper(ctx: RunContext[Any], *args: Any, **kwargs: Any) -> Any:
        return fn(*args, **kwargs)

    # Update signature and annotations
    wrapper.__signature__ = new_sig  # type: ignore
    wrapper.__annotations__ = {"ctx": RunContext[Any], **fn.__annotations__, "return": fn.__annotations__.get("return", str)}

    return wrapper


def create_docker_toolset() -> Any:
    """Create a prefixed FunctionToolset with all Docker tools + planning."""
    from pydantic_ai.toolsets import FunctionToolset, PrefixedToolset

    # Add a simple planning tool (no ctx needed - just returns confirmation)
    def create_plan(steps: str) -> str:
        """Create an execution plan from a JSON list of steps.

        Args:
            steps: JSON array of step descriptions. Example: ["Create network", "Run container"]

        Returns:
            Confirmation of plan creation.
        """
        try:
            parsed = _parse_json(steps, "steps")
            if not isinstance(parsed, list):
                return _error("steps must be a JSON array")

            plan_lines = [f"{i+1}. {step}" for i, step in enumerate(parsed)]
            return _ok(
    ...
    # Wrap planning tool with context
    plan_tool = _wrap_tool_for_context(create_plan)

    wrapped_tools = [_wrap_tool_for_context(t) for t in ALL_DOCKER_TOOLS] + [plan_tool]
    toolset = FunctionToolset(wrapped_tools, id="docker")
    return PrefixedToolset(toolset, prefix="docker")
...
class RunContainerInput(BaseModel):
    ...
    ports: str | None = Field(
        default=None,
        description='Port mapping: "HOST_PORT:CONTAINER_PORT" (e.g., "9000:80"). Container port 80 exposed on host port 9000.',
    )
...
def run_container(
    ...
    try:
        ...
        if ports:
            kwargs["ports"] = _parse_ports(ports)
        if environment:
...
__all__ = [
    ...
    "COMPOSE_TOOLS",
    "ALL_DOCKER_TOOLS",
    "register_tools_on_agent",
    "wrap_tools_for_pydantic",
    "create_docker_toolset",
]
